...
By calling React.useEffect without a second parameter, the code inside useEffectwill be triggered right when the component is **
just mounted and on any update**
...
asynchronous call, the closure principle will be applied and if we need to retrieve any state data it will just hold a frozen
value from the past.
